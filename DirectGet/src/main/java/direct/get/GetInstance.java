...
public final class GetInstance {
	...
	private final GetInstance parentGet;
	
	@SuppressWarnings("rawtypes")
	private final Map<Ref, Stack<Providing>> providingStacks = new TreeMap<>();
	
	GetInstance(GetInstance parentGet, RefSpace refSpace) {
	...
	public <T> GetInstance substitute(Providing<T> providing, Runnable runnable) {
		if (providing == null) {
			runnable.run();
		} else {
